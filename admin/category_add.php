...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['submit']) && isset($_FILES['file'])) {
        ...
        $file = $_FILES['file'];
        $strFileName = pathinfo($_FILES['file']['name'], PATHINFO_FILENAME);
        $allowed_ext = array("jpg", "jpeg", "JPG", "JPEG", "png", "PNG");
        $combine = $strFileName . "jpg";

        if ($cat != "" && $desc != "" && empty($file) == false) {

            $ext = explode(".", $_FILES['file']['name']);

            if (in_array($ext[1], $allowed_ext)) {

                $check = $db->query("SELECT * FROM category WHERE category_name='" . $cat . "' LIMIT 1");

                if ($check->num_rows) {
                    $msg = "<p style='color:red; padding: 10px; background: #ffeeee;'>No duplicate  food name allowed. Try again!!!</p>";
                } else {

                    $insert = $db->query("INSERT INTO category(category_name, category_description) VALUES('" . $cat . "', '" . $desc . "')");

                    if ($insert) {

                        $ins_id = $db->insert_id;
                        $nameImg = $ins_id . ".jpg";
                        $image_url = "../uploads/images/category/$ins_id.jpg";
                        $db->query("UPDATE category SET `img_category`='$nameImg' WHERE `id_category`='$ins_id'");
                        if (move_uploaded_file($_FILES['file']['tmp_name'], $image_url)) {

                            $msg = "<p style='color:green; padding: 10px; background: #eeffee;'>Category record successfully saved</p>";
                        } else {
                ...
                <nav class="navbar navbar-default navbar-fixed" style="background: #FF5722;">
                    <div class="container-fluid">
                        <div class="navbar-header">
                            ...
                            <a class="navbar-brand" href="#" style="color: #fff;">ADD NEW CATEGORY</a>
                        </div>
                    ...
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-md-8">
                                <div class="card">
                                    <div class="header">
                                        <h4 class="title">Category Add Form</h4>
                                    </div>
                                    <div class="content">
                                        <form method="post" action="category_add.php" enctype="multipart/form-data">
                                            <div class="row">
                                                <!-- <div class="col-md-12">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <option value="stickyrice">Sticky</option>
                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" autocomplete="off" name="category" class="form-control" placeholder="Enter Category Name" required />
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <label style="color: #333" for="txtarea">Category Description</label>
                                                        <textarea id="txtarea" class="form-control" placeholder="Enter Category description" name="desc" required></textarea>
                                                    </div>
                                                </div>
                                            </div>
